...
class ComTable extends React.Component {
  ...
  handleDelete = (text) => {
    Modal.confirm({
      title: '删除确认',
      content: `确定要删除【${text['domain']}】?`,
      onOk: () => {
        return http.delete('/api/config/domainlist/', {params: {id: text.id}})
          .then(() => {
  ...
  render() {
    ...
    if (store.f_name) {
      data = data.filter(item => item['domain'].toLowerCase().includes(store.f_name.toLowerCase()))
    }
    return (
      <React.Fragment>
        <Table
