...
class ComTable extends React.Component {
  ...
  handleDelete = (text) => {
    Modal.confirm({
      title: '删除确认',
      content: `确定要删除【${text['ipaddress']}】?`,
      onOk: () => {
        return http.delete('/api/config/portlist/', {params: {id: text.id}})
          .then(() => {
  ...
  render() {
    ...
    if (store.f_name) {
      data = data.filter(item => item['ipaddress'].toLowerCase().includes(store.f_name.toLowerCase()))
    }
    return (
      <React.Fragment>
        <Table
          ...
          <Table.Column title="ip地址" dataIndex="ipaddress"/>
          <Table.Column title="端口" dataIndex="port"/>
          <Table.Column ellipsis title="描述信息" dataIndex="comment"/>
