...
class HostView(View):
    def get(self, request,tag):
        ...
        if tag =="host":
            hosts = Host.objects.filter(resource_type=(ResourceType.objects.get(name='主机')).id).all().exclude(ipaddress="")
...
class System_Check(View):
    def get(self,request):
        xx = SystemCheck.objects.filter(check_time=str(time.strftime("%Y-%m-%d", time.localtime()))).all()
        hosts = SystemHealthInfo.objects.filter(check_time=str(time.strftime("%Y-%m-%d", time.localtime()))).all()
        if len(xx) >0:
            return json_response({"data": [x.to_dict() for x in xx],"hostdata": [x.to_dict() for x in hosts]})
        else:
    ...
    def post(self,request):
        form, error = JsonParser(
            ...
            Argument('url', type=str, required=False),
        ).parse(request.body)
        if error is None:
            if SystemCheck.objects.filter(name=form.name).exists() or SystemCheck.objects.filter(url=form.url).exists():
                return json_response(error="已存在")
    ...
    def delete(self,request):
        ...
        if error is None:
            SystemCheck.objects.filter(pk=form.id).delete()
        return json_response(error=error)