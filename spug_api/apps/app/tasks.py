...
def updatetoken():
    try:
        ...
        kwargs['url'] = "http://19.104.50.128:8011/api/account/login/"
        kwargs['data'] = json.dumps({"username": "spug", "password": "ALLTosea20181111", "type": "default"})
        logger.info(msg="update admin deploy status start")
        res = RequestApiAgent().create(**kwargs)
        logger.info(msg="update admin deploy status ->" + str(json.loads(res.content)))
